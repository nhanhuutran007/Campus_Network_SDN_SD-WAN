...
\begin{figure}[H]
    ...
    \begin{tikzpicture}[
        ...
        \node[controller] (s900) at (0,5.1) {\textbf{Site 900 -- Control plane SD-WAN}\\
        vManage \texttt{10.9.0.10}; vSmart \texttt{10.9.0.11} (System-IP \texttt{10.9.0.13}); vBond \texttt{10.9.0.12}\\
        vEdge65 staging: System-IP \texttt{10.200.90.1} -- không thuộc tập tám edge nghiệm thu};
        ...
        vEdge1/2: \texttt{10.200.100.1/.2}, mỗi edge có Internet và MPLS TLOC\\
        ...
        \textcolor{purple!75!black}{- - - $\triangleright$} OMP/control connection};
    \end{tikzpicture}%
    }
    \caption{Topology logic đã chuẩn hóa theo cấu hình hiện hành}
    \label{fig:topology_mang_campus}
\end{figure}

\noindent Sơ đồ vector trên được dựng lại từ tệp \texttt{Campus Network SDN SD-WAN.unl}, cây \texttt{configs/} và bảng quy hoạch hiện hành. Các ảnh xuất từ topology ở giai đoạn thiết kế được giữ trong thư mục lưu trữ nhưng không dùng làm bằng chứng địa chỉ trong báo cáo.
